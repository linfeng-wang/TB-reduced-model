...
training_df["Seq"] = seqs_df['KatG']
training_df['Dr'] = res_all["ISONIAZID"]
training_df = training_df.dropna()

seqs_df_agg =  training_df['Seq'].values.tolist()
# res_all_combined = res_all.values.tolist()
res_all_combined = training_df['Dr'].values.tolist()
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.8, patience=2,min_lr=1e-6, verbose=True)
#scheduler = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9, dampening=0, weight_decay=0, nesterov=False, maximize=False, foreach=None)
#scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=1.2)
#lambda1 = lambda epoch: 1.5 ** epoch
